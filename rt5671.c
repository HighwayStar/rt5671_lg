...
#define VERSION "0.0.1 alsa 1.0.24"

static int lout_on = 0; /* Oder 20130321 */
static int hp_on = 0; /* Oder 20130530 */
...
static struct rt5671_init_reg init_list[] = {
	...
	{RT5671_PRIV_DATA	, 0x3600},
	/*playback*/
//	{RT5671_STO_DAC_MIXER	, 0x1616},	/*Dig inf 1 -> Sto DAC mixer -> DACL */ //Oder_0605_voicecall
...
/*	{RT5671_DD_MIXER	, 0x1414}, */
	...
	{RT5671_GEN_CTRL2	, 0x0723},	// Oder 20130522
#if 0 /* test IF_ADC3 */
...
static const u16 rt5671_reg[RT5671_VENDOR_ID2 + 1] = {
	...
	[RT5671_ADDA_CLK1] = 0x1110,
	[RT5671_ADDA_CLK2] = 0x0e00,
	...
	[RT5671_EQ_CTRL1] = 0x6000,
	[RT5671_ALC_DRC_CTRL2] = 0x001f,
	[RT5671_ALC_CTRL_1] = 0x2206,
	[RT5671_ALC_CTRL_2] = 0x1f00,
	[RT5671_ANC_CTRL1] = 0x034b,
	[RT5671_ANC_CTRL2] = 0x0066,
	[RT5671_ANC_CTRL3] = 0x000b,
	[RT5671_BASE_BACK] = 0x1813,
	[RT5671_MP3_PLUS1] = 0x0690,
	[RT5671_MP3_PLUS2] = 0x1c17,
	[RT5671_ADJ_HPF1] = 0xb320,
	[RT5671_HP_CALIB_AMP_DET] = 0x0400,
	[RT5671_SV_ZCD1] = 0x0809,
	[RT5671_IL_CMD] = 0x0001,
	[RT5671_IL_CMD2] = 0x0049,
	[RT5671_IL_CMD3] = 0x0009,
	[RT5671_DRC_HL_CTRL1] = 0x8000,
	[RT5671_ADC_MONO_HP_CTRL1] = 0xb300,
	[RT5671_ADC_STO2_HP_CTRL1] = 0xb300,
	[RT5671_DIG_MISC] = 0x0090,
};
	...
	struct snd_soc_codec *codec, unsigned int reg)
{
	...
	case RT5671_ANC_CTRL1:
	case RT5671_IRQ_CTRL2:
	case RT5671_IRQ_CTRL3:
	case RT5671_INT_IRQ_ST:
	case RT5671_IL_CMD:
	...
	struct snd_soc_codec *codec, unsigned int reg)
{
	...
	case RT5671_ANC_CTRL1:
	case RT5671_ANC_CTRL2:
	case RT5671_ANC_CTRL3:
	case RT5671_JD_CTRL:
...
{
	int jack_type, val, reg61, reg63, reg64, regfa, reg80, regbd;	// Oder 130617
	...
	if(jack_insert) {
		...
		snd_soc_update_bits(codec, RT5671_CJ_CTRL2,
			...
			RT5671_CBJ_MN_JD);
		snd_soc_write(codec, RT5671_JD_CTRL3, 0x0008);
...
//			RT5671_GP1_PIN_MASK, RT5671_GP1_PIN_IRQ);
		...
		snd_soc_update_bits(codec, RT5671_PWR_DIG1,
			...
			RT5671_PWR_DAC_L1 | RT5671_PWR_DAC_R1);
//		regbd = snd_soc_read(codec, RT5671_IRQ_CTRL2);	// Oder 130617
//		snd_soc_write(codec, RT5671_IRQ_CTRL2, 0x0080);	// Oder 130617
		...
		snd_soc_write(codec, RT5671_JD_CTRL3, 0x0088);

		msleep(500); // Oder 130618
		val = snd_soc_read(codec, RT5671_CJ_CTRL3) & 0x7;
		pr_debug("val=%d\n",val);
		if ( val == 0x1 || val == 0x2)
			jack_type = RT5671_HEADSET_DET;
		else
			jack_type = RT5671_HEADPHO_DET;
...
//		snd_soc_write(codec, RT5671_IRQ_CTRL2, regbd);	// Oder 130617
//		snd_soc_update_bits(codec, RT5671_CJ_CTRL2,		// Oder 130806
//			RT5671_CBJ_DET_MODE, RT5671_CBJ_DET_MODE);	// Oder 130806
	} else {
		jack_type = RT5671_NO_JACK;
	}
...
int rt5671_check_interrupt_event(struct snd_soc_codec *codec)
{
	int val;

	val = snd_soc_read(codec, RT5671_A_JD_CTRL1) & 0x0010;
	if (val)
		snd_soc_update_bits(codec, RT5671_IRQ_CTRL2, /* change jd polarity */
			    0x1 << 7, 0x1 << 7);
	else
		snd_soc_update_bits(codec, RT5671_IRQ_CTRL2, /* change jd polarity */
			    0x1 << 7, 0);
...
/* IN1/IN2 Input Type */
static const char *rt5671_input_mode[] = {
...
static const char *rt5671_data_select[] = {
...
/* DMIC */
/*
static const char *rt5671_dmic_mode[] = {"Disable", "DMIC1", "DMIC2"};

static const SOC_ENUM_SINGLE_DECL(rt5671_dmic_enum, 0, 0, rt5671_dmic_mode);
*/
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.integer.value[0] == 0) {
		snd_soc_update_bits(codec, RT5671_ASRC_2 ,0xf000, 0x0000);
		snd_soc_update_bits(codec, RT5671_ASRC_10 ,0xf000, 0x0000);
	} else {
		snd_soc_update_bits(codec, RT5671_ASRC_2 ,0xf000, 0x6000);
...
static const struct snd_kcontrol_new rt5671_snd_controls[] = {
	...
	SOC_DOUBLE_TLV("STO2 ADC Boost Gain", RT5671_ADC_BST_VOL1,
			...
			3, 0, adc_bst_tlv),
	SOC_ENUM_EXT("VAD Switch", rt5671_vad_enum,	// Oder 130620
		rt5671_vad_get, rt5671_vad_put),	// Oder 130620
	/* Oder 130904 Start */
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	struct rt5671_priv *rt5671 = snd_soc_codec_get_drvdata(codec);
	int div[] = {2, 3, 4, 6, 8, 12}, idx = -EINVAL, i, rate, red, bound, temp; // Oder 130701
	...
	if (idx < 0)
		...
		snd_soc_update_bits(codec, RT5671_DMIC_CTRL1, RT5671_DMIC_CLK_MASK,
					idx << RT5671_DMIC_CLK_SFT);
	return idx;
}
...
static const struct snd_kcontrol_new rt5671_mono_dac_l_mix[] = {
	SOC_DAPM_SINGLE("DAC L1 Switch", RT5671_DD_MIXER,
			RT5671_M_DAC_L1_MONO_L_SFT, 1, 1),
	SOC_DAPM_SINGLE("DAC L2 Switch", RT5671_DD_MIXER,
			RT5671_M_DAC_L2_MONO_L_SFT, 1, 1),
	SOC_DAPM_SINGLE("DAC R2 Switch", RT5671_DD_MIXER,
...
static const struct snd_kcontrol_new rt5671_mono_dac_r_mix[] = {
	SOC_DAPM_SINGLE("DAC R1 Switch", RT5671_DD_MIXER,
...
static const struct snd_kcontrol_new rt5671_rec_l_mix[] = {
	SOC_DAPM_SINGLE("HPOL Switch", RT5671_REC_L2_MIXER,
			RT5671_M_HP_L_RM_L_SFT, 1, 1),
	...
	SOC_DAPM_SINGLE("OUT MIXL Switch", RT5671_REC_L2_MIXER,
			RT5671_M_OM_L_RM_L_SFT, 1, 1),
};

static const struct snd_kcontrol_new rt5671_rec_r_mix[] = {
	SOC_DAPM_SINGLE("HPOR Switch", RT5671_REC_R2_MIXER,
			RT5671_M_HP_R_RM_R_SFT, 1, 1),
	...
	SOC_DAPM_SINGLE("OUT MIXR Switch", RT5671_REC_R2_MIXER,
			RT5671_M_OM_R_RM_R_SFT, 1, 1),
};
...
static const struct snd_kcontrol_new rt5671_rec_m_mix[] = {
	...
	SOC_DAPM_SINGLE("OUT MIXL Switch", RT5671_REC_MONO2_MIXER,
			RT5671_M_OM_L_RM_M_SFT, 1, 1),
};
...
static const char *rt5671_dac1_src[] = {"IF1 DAC", "IF2 DAC", "IF3 DAC", "IF4 DAC"};
...
static const char *rt5671_dac12_src[] = {"IF1 DAC", "IF2 DAC", "IF3 DAC", "TxDC DAC", "Bass", "VAD_ADC", "IF4 DAC"};

static const SOC_ENUM_SINGLE_DECL(
	rt5671_dac2l_enum, RT5671_DAC_CTRL,
	RT5671_DAC2_L_SEL_SFT, rt5671_dac12_src);

static const struct snd_kcontrol_new rt5671_dac_l2_mux =
	SOC_DAPM_ENUM("DAC2 L source", rt5671_dac2l_enum);

static const char *rt5671_dacr2_src[] = {"IF1 DAC", "IF2 DAC", "IF3 DAC", "TxDC DAC", "TxDP ADC", "IF4 DAC"};
...
static const char *rt5671_rxdp_src[] = {"IF2 DAC", "IF1 DAC", "STO1 ADC Mixer", "STO2 ADC Mixer", "Mono ADC Mixer L", "Mono ADC Mixer R", "DAC1"};
...
/* MX-2D [1] [0] */
static const char *rt5671_dsp_bypass_src[] = {"DSP", "Bypass"};
...
/* INL/R source */
static const char *rt5671_inl_src[] = {"IN2P", "MonoP"};
...
static const char *rt5671_inr_src[] = {"IN2N", "MonoN"};
...
/* Stereo2 ADC source */
/* MX-26 [15] */
static const char *rt5671_stereo2_adc_lr_src[] = {"L", "LR"};
...
/* Stereo1 ADC source */
/* MX-27 MX-26 [12] */
static const char *rt5671_stereo_adc1_src[] = {"DAC MIX", "ADC"};
...
/* MX-27 MX-26 [11] */
static const char *rt5671_stereo_adc2_src[] = {"DAC MIX", "DMIC"};
...
/* MX-27 MX26 [10] */
static const char *rt5671_stereo_adc_src[] = {"ADC1L ADC2R", "ADC3"};
...
//MX-27 MX-26 [9:8]
static const char *rt5671_stereo_dmic_src[] = {"DMIC1", "DMIC2", "DMIC3"};
...
//MX-27 [0]
static const char *rt5671_stereo_dmic3_src[] = {"DMIC3", "PDM ADC"};
...
/* Mono ADC source */
//MX-28 [12]
static const char *rt5671_mono_adc_l1_src[] = {"Mono DAC MIXL", "ADC1 ADC3"};
...
//MX-28 [11]
static const char *rt5671_mono_adc_l2_src[] =
	{"Mono DAC MIXL", "DMIC"};
...
//MX-28 [10]
static const char *rt5671_mono_adc_l_src[] =
	{"ADC1", "ADC3"};
...
//MX-28 [9:8]
static const char *rt5671_mono_dmic_src[] =
	{"DMIC1", "DMIC2", "DMIC3"};
...
//MX-28 [4]
static const char *rt5671_mono_adc_r1_src[] = {"Mono DAC MIXR", "ADC2 ADC3"};
...
//MX-28 [3]
static const char *rt5671_mono_adc_r2_src[] =
	{"Mono DAC MIXR", "DMIC"};
...
//MX-28 [2]
static const char *rt5671_mono_adc_r_src[] =
	{"ADC2", "ADC3"};
...
//MX-2F [15]
static const char *rt5671_if1_adc2_in_src[] =
	{"IF_ADC2", "VAD_ADC"};
...
//MX-2F [14:12]
static const char *rt5671_if2_adc_in_src[] =
...
//MX-2F [2:0]
static const char *rt5671_if3_adc_in_src[] =
	{"IF_ADC1", "IF_ADC2", "IF_ADC3", "TxDC_DAC", "TxDP_ADC", "VAD_ADC"};
...
//MX-30 [5:4]
static const char *rt5671_if4_adc_in_src[] =
...
//MX-31 [15] [13] [11] [9]
static const char *rt5671_pdm_src[] =
	{"Mono DAC", "Stereo DAC"};
...
//MX-FA [12]
static const char *rt5671_if1_adc1_in1_src[] =
	{"IF_ADC1", "IF1_ADC3"};

static const SOC_ENUM_SINGLE_DECL(
	rt5671_if1_adc1_in1_enum, RT5671_DIG_MISC,
...
//MX-FA [11]
static const char *rt5671_if1_adc1_in2_src[] =
	{"IF1_ADC1_IN1", "IF1_ADC4"};

static const SOC_ENUM_SINGLE_DECL(
	rt5671_if1_adc1_in2_enum, RT5671_DIG_MISC,
...
//MX-FA [10]
static const char *rt5671_if1_adc2_in1_src[] =
	{"IF1_ADC2_IN", "IF1_ADC4"};

static const SOC_ENUM_SINGLE_DECL(
	rt5671_if1_adc2_in1_enum, RT5671_DIG_MISC,
...
//MX-18 [11:9]
static const char *rt5671_sidetone_src[] =
...
//MX-18 [6]
static const char *rt5671_anc_src[] =
	{"SNC", "Sidetone"};
...
//MX-9D [9:8]
static const char *rt5671_vad_adc_src[] =
...
/*	dump_reg(codec); */
	if(on) {
		if(hp_amp_power_count <= 0) {
			...
			snd_soc_update_bits(codec, RT5671_PWR_VOL,
				RT5671_PWR_HV_L | RT5671_PWR_HV_R,
				RT5671_PWR_HV_L | RT5671_PWR_HV_R);
			snd_soc_update_bits(codec, RT5671_PWR_ANLG1,
				RT5671_PWR_HP_L | RT5671_PWR_HP_R | RT5671_PWR_HA |
				RT5671_PWR_FV1 | RT5671_PWR_FV2,
				RT5671_PWR_HP_L | RT5671_PWR_HP_R | RT5671_PWR_HA |
				RT5671_PWR_FV1 | RT5671_PWR_FV2);
			/* depop parameters */
#if 0
			snd_soc_update_bits(codec, RT5671_DEPOP_M2,
				RT5671_DEPOP_MASK, RT5671_DEPOP_MAN);
			snd_soc_update_bits(codec, RT5671_DEPOP_M1,
				RT5671_HP_CP_MASK | RT5671_HP_SG_MASK | RT5671_HP_CB_MASK,
				RT5671_HP_CP_PU | RT5671_HP_SG_DIS | RT5671_HP_CB_PU);
#else
...
#if 0
			snd_soc_update_bits(codec, RT5671_DEPOP_M1,
				RT5671_HP_CO_MASK | RT5671_HP_SG_MASK,
				RT5671_HP_CO_EN | RT5671_HP_SG_EN);
#else
...
#if 0
			snd_soc_update_bits(codec, RT5671_DEPOP_M1,
				RT5671_HP_SG_MASK | RT5671_HP_L_SMT_MASK |
				RT5671_HP_R_SMT_MASK, RT5671_HP_SG_DIS |
				RT5671_HP_L_SMT_DIS | RT5671_HP_R_SMT_DIS);
			/* headphone amp power down */
			snd_soc_update_bits(codec, RT5671_DEPOP_M1,
				RT5671_SMT_TRIG_MASK | RT5671_HP_CD_PD_MASK |
				RT5671_HP_CO_MASK | RT5671_HP_CP_MASK |
				RT5671_HP_SG_MASK | RT5671_HP_CB_MASK,
				RT5671_SMT_TRIG_DIS | RT5671_HP_CD_PD_EN |
				RT5671_HP_CO_DIS | RT5671_HP_CP_PD |
				RT5671_HP_SG_EN | RT5671_HP_CB_PD);
#endif
...
#if 0
	snd_soc_update_bits(codec, RT5671_DEPOP_M3,
		RT5671_CP_FQ1_MASK | RT5671_CP_FQ2_MASK | RT5671_CP_FQ3_MASK,
		(RT5671_CP_FQ_192_KHZ << RT5671_CP_FQ1_SFT) |
		(RT5671_CP_FQ_96_KHZ << RT5671_CP_FQ2_SFT) |
		(RT5671_CP_FQ_48_KHZ << RT5671_CP_FQ3_SFT));
	rt5671_index_write(codec, RT5671_MAMP_INT_REG2, 0xfc00);
	snd_soc_update_bits(codec, RT5671_DEPOP_M1,
		RT5671_SMT_TRIG_MASK, RT5671_SMT_TRIG_EN);
	snd_soc_update_bits(codec, RT5671_DEPOP_M1,
		RT5671_RSTN_MASK, RT5671_RSTN_EN);
	snd_soc_update_bits(codec, RT5671_DEPOP_M1,
		RT5671_RSTN_MASK | RT5671_HP_L_SMT_MASK | RT5671_HP_R_SMT_MASK,
		RT5671_RSTN_DIS | RT5671_HP_L_SMT_EN | RT5671_HP_R_SMT_EN);
#endif
...
static void rt5671_pmd_depop(struct snd_soc_codec *codec)
{
	...
	snd_soc_update_bits(codec, RT5671_DEPOP_M3,
		RT5671_CP_FQ1_MASK | RT5671_CP_FQ2_MASK | RT5671_CP_FQ3_MASK,
		(RT5671_CP_FQ_96_KHZ << RT5671_CP_FQ1_SFT) |
		(RT5671_CP_FQ_12_KHZ << RT5671_CP_FQ2_SFT) |
		(RT5671_CP_FQ_96_KHZ << RT5671_CP_FQ3_SFT));
#else
	snd_soc_write(codec, RT5671_DEPOP_M1, 0x8019);
...
#if 0
	snd_soc_update_bits(codec, RT5671_DEPOP_M1,
		RT5671_HP_SG_MASK, RT5671_HP_SG_EN);
	snd_soc_update_bits(codec, RT5671_DEPOP_M1,
		RT5671_RSTP_MASK, RT5671_RSTP_EN);
	snd_soc_update_bits(codec, RT5671_DEPOP_M1,
		RT5671_RSTP_MASK | RT5671_HP_L_SMT_MASK |
		RT5671_HP_R_SMT_MASK, RT5671_RSTP_DIS |
		RT5671_HP_L_SMT_EN | RT5671_HP_R_SMT_EN);
#else
	if (lout_on == 0) // Oder 20130321
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		hp_on = 1; // Oder 20130530
		//rt5671_update_eqmode(codec, EQ_CH_ADC, HP);
	...
	case SND_SOC_DAPM_PRE_PMD:
		...
		msleep(200);
		snd_soc_update_bits(codec, RT5671_CJ_CTRL2,
				RT5671_CBJ_DET_MODE, 0);

		//snd_soc_update_bits(codec, RT5671_CJ_CTRL1,
		//		RT5671_CBJ_BST1_EN, 0);
...
#ifdef RT5672
static int rt5671_spk_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = w->codec;
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		//printk("rt5671_spk_event SND_SOC_DAPM_POST_PMU\n");
		rt5671_pdm1_write(codec, 0x09, 0x0003);
		rt5671_pdm1_write(codec, 0x0e, 0x0100);
		rt5671_pdm1_write(codec, 0x01, 0x9a45);
		rt5671_pdm1_write(codec, 0x05, 0x06f5);
		rt5671_pdm1_write(codec, 0x04, 0x98a2);
		rt5671_pdm1_write(codec, 0x08, 0x3016);
	break;

	case SND_SOC_DAPM_PRE_PMD:
		rt5671_pdm1_write(codec, 0x09, 0x0002);
		break;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		lout_on = 1; // Oder 20130321
		snd_soc_write(codec, RT5671_DEPOP_M1, 0x0019); // Oder 20130321
		snd_soc_write(codec, RT5671_DEPOP_M2, 0x3100); // Oder 20130321
		snd_soc_update_bits(codec, RT5671_CHARGE_PUMP,
			RT5671_PM_HP_MASK, RT5671_PM_HP_HV); // Oder 20130321-1
		snd_soc_update_bits(codec, RT5671_LOUT1,
			RT5671_L_MUTE | RT5671_R_MUTE, 0);
		break;

	case SND_SOC_DAPM_PRE_PMD:
		lout_on = 0; // Oder 20130321
		if (hp_on == 0) { // Oder 20130530
			snd_soc_write(codec, RT5671_DEPOP_M1, 0x0004); // Oder 20130321
			snd_soc_write(codec, RT5671_DEPOP_M2, 0x1100); // Oder 20130321
		}
		snd_soc_update_bits(codec, RT5671_LOUT1,
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		snd_soc_update_bits(codec, RT5671_GPIO_CTRL1,
			...
			RT5671_GP2_PIN_DMIC1_SCL | RT5671_GP7_PIN_DMIC1_SDA);	// Oder 130701
		snd_soc_update_bits(codec, RT5671_DMIC_CTRL1,
			...
			RT5671_DMIC_1_DP_GPIO7);				// Oder 130701
		snd_soc_update_bits(codec, RT5671_DMIC_CTRL1,
...
static int rt5671_set_dmic3_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event) //bard: need to modify
{
	struct snd_soc_codec *codec = w->codec;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMD:
		snd_soc_update_bits(codec, RT5671_CJ_CTRL2,
				RT5671_CBJ_DET_MODE, RT5671_CBJ_DET_MODE);
		//snd_soc_update_bits(codec, RT5671_PWR_ANLG2,
		//	RT5671_PWR_MB1 | RT5671_PWR_JD1, 0);
		msleep(200);
		snd_soc_update_bits(codec, RT5671_PWR_ANLG2,
			RT5671_PWR_BST1_P, 0);
		mdelay(20);
...
static int rt5671_ldo_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = w->codec;
	...
	case SND_SOC_DAPM_POST_PMD:
		snd_soc_update_bits(codec, RT5671_PWR_ANLG1, RT5671_LDO_SEL_MASK, 0x0);
		break;
...
static const struct snd_soc_dapm_widget rt5671_dapm_widgets[] = {
	...
	/* Input Side */
	/* micbias */
	SND_SOC_DAPM_MICBIAS("micbias1", RT5671_PWR_ANLG2, // Oder 20130523
	...
	SND_SOC_DAPM_PGA_E("DMIC1", SND_SOC_NOPM, 0, 0, NULL, 0,
		rt5671_set_dmic1_event, SND_SOC_DAPM_PRE_PMU),
	SND_SOC_DAPM_PGA_E("DMIC2", SND_SOC_NOPM, 0, 0, NULL, 0,
		rt5671_set_dmic2_event, SND_SOC_DAPM_PRE_PMU),
	SND_SOC_DAPM_PGA_E("DMIC3", SND_SOC_NOPM, 0, 0, NULL, 0,
		rt5671_set_dmic3_event, SND_SOC_DAPM_PRE_PMU),
	...
	SND_SOC_DAPM_ADC("ADC 2", NULL, SND_SOC_NOPM,
		0, 0),
	SND_SOC_DAPM_ADC("ADC 3", NULL, SND_SOC_NOPM,
		0, 0),
	...
	SND_SOC_DAPM_SUPPLY("ADC clock",SND_SOC_NOPM, 0, 0,
		rt5671_adc_clk_event, SND_SOC_DAPM_POST_PMD | SND_SOC_DAPM_POST_PMU),
	/* ADC Mux */
	...
	SND_SOC_DAPM_MIXER_E("Sto1 ADC MIXL", SND_SOC_NOPM, 0, 0,
		...
		rt5671_sto1_adcl_event, SND_SOC_DAPM_PRE_PMD | SND_SOC_DAPM_POST_PMU),
	SND_SOC_DAPM_MIXER_E("Sto1 ADC MIXR", SND_SOC_NOPM, 0, 0,
		rt5671_sto1_adc_r_mix, ARRAY_SIZE(rt5671_sto1_adc_r_mix),
		rt5671_sto1_adcr_event, SND_SOC_DAPM_PRE_PMD | SND_SOC_DAPM_POST_PMU),
	...
	SND_SOC_DAPM_MIXER_E("Mono ADC MIXL", SND_SOC_NOPM, 0, 0,
		...
		rt5671_mono_adcl_event, SND_SOC_DAPM_PRE_PMD | SND_SOC_DAPM_POST_PMU),
	...
	SND_SOC_DAPM_MIXER_E("Mono ADC MIXR", SND_SOC_NOPM, 0, 0,
		...
		rt5671_mono_adcr_event, SND_SOC_DAPM_PRE_PMD | SND_SOC_DAPM_POST_PMU),
	...
	SND_SOC_DAPM_MUX("DSP UL Mux", SND_SOC_NOPM, 0, 0, &rt5671_dsp_ul_mux),
	SND_SOC_DAPM_MUX("DSP DL Mux", SND_SOC_NOPM, 0, 0, &rt5671_dsp_dl_mux),

	SND_SOC_DAPM_MUX("RxDP Mux", SND_SOC_NOPM, 0, 0, &rt5671_rxdp_mux),

	/* IF2 3 4 Mux */
	SND_SOC_DAPM_MUX("IF2 ADC Mux", SND_SOC_NOPM, 0, 0, &rt5671_if2_adc_in_mux),
	SND_SOC_DAPM_MUX("IF3 ADC Mux", SND_SOC_NOPM, 0, 0, &rt5671_if3_adc_in_mux),
	SND_SOC_DAPM_MUX("IF4 ADC Mux", SND_SOC_NOPM, 0, 0, &rt5671_if4_adc_in_mux),
	...
	SND_SOC_DAPM_MUX("IF1 ADC1 IN1 Mux", SND_SOC_NOPM, 0, 0, &rt5671_if1_adc1_in1_mux),
	SND_SOC_DAPM_MUX("IF1 ADC1 IN2 Mux", SND_SOC_NOPM, 0, 0, &rt5671_if1_adc1_in2_mux),
	SND_SOC_DAPM_MUX("IF1 ADC2 IN Mux", SND_SOC_NOPM, 0, 0, &rt5671_if1_adc2_in_mux),
	SND_SOC_DAPM_MUX("IF1 ADC2 IN1 Mux", SND_SOC_NOPM, 0, 0, &rt5671_if1_adc2_in1_mux),

	SND_SOC_DAPM_MUX("VAD ADC Mux", SND_SOC_NOPM, 0, 0, &rt5671_vad_adc_mux),
	...
	SND_SOC_DAPM_DAC("DAC R1", NULL, RT5671_PWR_DIG1,
			RT5671_PWR_DAC_R1_BIT, 0),
	...
	SND_SOC_DAPM_MIXER("LOUT MIX", RT5671_PWR_ANLG1, RT5671_PWR_LM_BIT, 0,
		rt5671_lout_mix, ARRAY_SIZE(rt5671_lout_mix)),
	SND_SOC_DAPM_MIXER("MONOVOL MIX", RT5671_PWR_ANLG1, RT5671_PWR_MM_BIT, 0,
		rt5671_mono_mix, ARRAY_SIZE(rt5671_mono_mix)),
	SND_SOC_DAPM_MIXER("MONOAmp MIX", SND_SOC_NOPM, 0, 0,
		rt5671_monoamp_mix, ARRAY_SIZE(rt5671_monoamp_mix)),

	SND_SOC_DAPM_PGA_S("HP amp", 1, SND_SOC_NOPM, 0, 0,
		rt5671_hp_event, SND_SOC_DAPM_PRE_PMD | SND_SOC_DAPM_POST_PMU),
	SND_SOC_DAPM_PGA_S("LOUT amp", 1, SND_SOC_NOPM, 0, 0,
		rt5671_lout_event, SND_SOC_DAPM_PRE_PMD | SND_SOC_DAPM_POST_PMU),
	...
	SND_SOC_DAPM_PGA_S("SPO amp", 2, SND_SOC_NOPM, 0, 0,
		rt5671_spk_event, SND_SOC_DAPM_PRE_PMD | SND_SOC_DAPM_POST_PMU),
#endif
	...
	SND_SOC_DAPM_MUX_E("PDM1 L Mux", SND_SOC_NOPM, 0, 0, &rt5671_pdm1_l_mux,
		rt5671_pdm1_l_event, SND_SOC_DAPM_PRE_PMD | SND_SOC_DAPM_POST_PMU),
	SND_SOC_DAPM_MUX_E("PDM1 R Mux", SND_SOC_NOPM, 0, 0, &rt5671_pdm1_r_mux,
		rt5671_pdm1_r_event, SND_SOC_DAPM_PRE_PMD | SND_SOC_DAPM_POST_PMU),
	SND_SOC_DAPM_MUX_E("PDM2 L Mux", SND_SOC_NOPM, 0, 0, &rt5671_pdm2_l_mux,
		rt5671_pdm2_l_event, SND_SOC_DAPM_PRE_PMD | SND_SOC_DAPM_POST_PMU),
	SND_SOC_DAPM_MUX_E("PDM2 R Mux", SND_SOC_NOPM, 0, 0, &rt5671_pdm2_r_mux,
		rt5671_pdm2_r_event, SND_SOC_DAPM_PRE_PMD | SND_SOC_DAPM_POST_PMU),
...
static const struct snd_soc_dapm_route rt5671_dapm_routes[] = {
 ...
 /* Oder 131003 Fix Typo Start */
	{ "DAC R2 Mux", "IF4 DAC", "IF4 DAC R" },
 /* Oder 131003 Fix Typo End */
...
static int get_sdp_info(struct snd_soc_codec *codec, int dai_id)
{
	...
	val = snd_soc_read(codec, RT5671_I2S1_SDP);
	val = (val & RT5671_I2S_IF_MASK) >> RT5671_I2S_IF_SFT;
	switch (dai_id) {
	case RT5671_AIF1:
		ret |= RT5671_U_IF1;
		break;
	...
	struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	int pre_div = 0, bclk_ms = 0, frame_size = 0;
...
/*
 * LGE_CHANGE_S
 * Comment : frame size adjustment for odin audio i2s dai
 * 2013-06-24, hyunsoo.yoon@lge.com
 */
/* Oder 131009 Start */
#if 0
/* Oder 131009 End */
	...
	case 32:
		...
		break;
	default:
		dev_err(codec->dev, "please check the frame_size %d\n", frame_size);
		return -EINVAL;
	}
#endif
/* LGE_CHANGE_E */
	...
	/* Oder 130830 End */
	/* Oder 130911 Start */
	if (dai_sel & RT5671_U_IF4) {
		mask_clk = RT5671_I2S_BCLK_MS1_MASK | RT5671_I2S_PD1_MASK;
		val_clk = bclk_ms << RT5671_I2S_BCLK_MS1_SFT |
			pre_div << RT5671_I2S_PD1_SFT;
		snd_soc_update_bits(codec, RT5671_I2S4_SDP,
			RT5671_I2S_DL_MASK, val_len);
		printk("hw_param RT5671_U_IF4 bclk_ms = %d mask_clk=%d, val_clk=%d\n", bclk_ms, mask_clk, val_clk);
				...
				struct snd_soc_dai *dai)
{
	...
	rt5671->stream = substream->stream;	// Oder 130620
	return 0;
}
...
static ssize_t rt5671_index_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct rt5671_priv *rt5671 = i2c_get_clientdata(client);
	struct snd_soc_codec *codec = rt5671->codec;
	...
	if(i==count)
	{
		printk("0x%02x = 0x%04x\n",addr,rt5671_index_read(codec, addr));
	}
	else
	{
	...
	struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i <= RT5671_VENDOR_ID2; i++) {
		if (cnt + RT5671_REG_DISP_LEN >= PAGE_SIZE)
			break;
		//val = codec->hw_read(codec, i); //Oder 20130522
...
static ssize_t rt5671_codec_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct rt5671_priv *rt5671 = i2c_get_clientdata(client);
	struct snd_soc_codec *codec = rt5671->codec;
...
static int rt5671_set_bias_level(struct snd_soc_codec *codec,
			enum snd_soc_bias_level level)
{
	struct rt5671_priv *rt5671 = snd_soc_codec_get_drvdata(codec); // Oder 130620
	switch (level) {
...
// Oder 130620 start
		if (rt5671->vad_en) {
			...
			snd_soc_write(codec, RT5671_TDM_CTRL_1, 0x0c00);
			Oder 131009 End */
//			snd_soc_write(codec, RT5671_STO1_ADC_MIXER, 0x3820);  // Oder 130701 remove it
		}
// Oder 130620 end
		/* Oder 130820
		snd_soc_update_bits(codec, RT5671_PWR_ANLG1, RT5671_LDO_SEL_MASK, 0x3);
	...
	case SND_SOC_BIAS_STANDBY:
		...
		if (SND_SOC_BIAS_OFF == codec->dapm.bias_level) {
			snd_soc_update_bits(codec, RT5671_PWR_ANLG1,
				...
				RT5671_PWR_BG | RT5671_PWR_VREF2);
			msleep(10);
			snd_soc_update_bits(codec, RT5671_PWR_ANLG2,	// Oder 20130522
...
//			snd_soc_write(codec, RT5671_CJ_CTRL2, 0x08a7);
		...
		/* Oder 130820 */
		snd_soc_write(codec, RT5671_PWR_ANLG1, 0x2001);	// Oder 20130319
		snd_soc_write(codec, RT5671_PWR_ANLG2, 0x0004);	// Oder 20130319
		/* Oder 130820 start */
		snd_soc_update_bits(codec, RT5671_GLB_CLK,
			RT5671_SCLK_SRC_MASK, RT5671_SCLK_SRC_MCLK);
...
static int rt5671_probe(struct snd_soc_codec *codec)
{
	...
	rt5671->vad_en = false;	// Add for VAD test Oder 130620

/*
 * LGE_CHANGE_S
 * Comment : snd_soc_add_control is not used anymore
 * 2013-03-06, hyunsoo.yoon@lge.com
 */
#if 0
	snd_soc_add_controls(codec, rt5671_snd_controls,
...
/* LGE_CHANGE_E */
	...
	rt5671->dsp_sw = RT5671_DSP_NS;	// Oder 130815
...
#ifdef CONFIG_PM
static int rt5671_suspend(struct snd_soc_codec *codec, pm_message_t state)
{
	struct rt5671_priv *rt5671 = snd_soc_codec_get_drvdata(codec);
	rt5671_dsp_suspend(codec); //Oder 20130822 for suspend power debug
	/* Move to VAD mode */
	rt5671_set_bias_level(codec, SND_SOC_BIAS_OFF); //Oder 20130822 for suspend power debug
	//snd_soc_write(codec, RT5671_PWR_ANLG1, 0x0000);	// Oder 20130523
	//snd_soc_write(codec, RT5671_PWR_ANLG2, 0x0000);	// Oder 20130523
	return 0;
}
...
static void rt5671_shutdown(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	printk("enter %s\n",__func__);
}
