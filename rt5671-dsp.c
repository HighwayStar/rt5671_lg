...
#define RT5671_DSP_PATCH_NUM \
	(sizeof(rt5671_dsp_patch) / sizeof(rt5671_dsp_patch[0]))
 ...
 * Returns 0 for success or negative error code.
 */
static int rt5671_dsp_do_patch(struct snd_soc_codec *codec)
{
	int ret, i;

	for (i = 0; i < RT5671_DSP_PATCH_NUM; i++) {
		ret = snd_soc_write(codec, rt5671_dsp_patch[i][0],
			rt5671_dsp_patch[i][1]);
		if (ret < 0)
			goto patch_err;


		if (rt5671_dsp_patch[i][0] == 0xe0) {
			ret = rt5671_dsp_done(codec);
			if (ret < 0) {
				dev_err(codec->dev, "DSP is busy: %d\n", ret);
				goto patch_err;
